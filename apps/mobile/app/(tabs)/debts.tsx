...
export default function DebtsScreen() {
  ...
  const [editingPerson, setEditingPerson] = useState<MobilePerson | null>(null);
  ...
  async function savePerson() {
    ...
    const wasEditing = editingPerson !== null;
    const record: MobilePerson = editingPerson
      ? { ...editingPerson, displayName: personName.trim(), contact: personContact.trim() || null, note: personNote.trim() || null, updatedAt: new Date().toISOString() }
      : { id: Crypto.randomUUID(), displayName: personName.trim(), contact: personContact.trim() || null, note: personNote.trim() || null, updatedAt: new Date().toISOString() };
    ...
    setPersonName(''); setPersonContact(''); setPersonNote(''); setEditingPerson(null); setForm(wasEditing ? null : 'debt'); await done();
  }
  ...
  return <Screen scrollable><View style={styles.header}><View><Title>Debts</Title><Text style={styles.subtitle}>What you owe and what people owe you.</Text></View><Button size="compact" onPress={() => { setDebtDraft(blankDebt(people[0]?.id ?? '')); setForm('debt'); }}>New</Button></View>{pending > 0 && activeWorkspace === 'online' ? <Badge variant="outline">Pending sync</Badge> : null}
    {debts.length === 0 ? <Empty>Create your first debt to keep your full financial picture in one place.</Empty> : groups.map(([name, group]) => <View key={name} style={styles.group}><SectionTitle>{name}</SectionTitle>{group.map((debt) => { const state = debtState(debt, debtAdjustments, debtPayments); const active = debt.status === 'open' || debt.status === 'partially_paid'; return <Card key={debt.id}><View style={styles.row}><View style={styles.copy}><Text style={ui.listTitle}>{debt.direction === 'receivable' ? 'They owe you' : 'You owe them'}</Text><Text style={ui.listMeta}>Opened {debt.openedAt}{debt.dueDate ? ` · Due ${debt.dueDate}` : ''}</Text></View><Text style={[styles.amount, debt.direction === 'receivable' ? styles.income : styles.expense]}>{formatMoney(state.outstandingBalance, debt.currency)}</Text></View><View style={styles.meta}><Badge variant="muted">{debt.status.replace('_', ' ')}</Badge><Text style={ui.listMeta}>Principal {formatMoney(debt.originalPrincipal, debt.currency)}</Text></View>{debt.note ? <Text style={ui.listMeta}>{debt.note}</Text> : null}<View style={styles.actions}>{active ? <><Button size="compact" variant="outline" onPress={() => { setSelectedDebt(debt); setForm('payment'); }}>Payment</Button><Button size="compact" variant="outline" onPress={() => { setSelectedDebt(debt); setForm('adjustment'); }}>Adjust</Button><Button size="compact" variant="outline" onPress={() => void changeStatus(debt, 'written_off')}>Write off</Button></> : <><Button size="compact" variant="outline" onPress={() => void changeStatus(debt, 'open')}>Reopen</Button><Button size="compact" variant="outline" onPress={() => void changeVisibility(debt, true)}>Hide</Button></>}</View></Card>; })}</View>)}
    {hiddenDebts.length > 0 ? <View style={styles.group}><SectionTitle>Hidden debts</SectionTitle><Text style={ui.listMeta}>Closed debts remain in your ledger and dashboard totals. Unhide one to return it to the normal list.</Text>{hiddenDebts.map(renderDebt)}</View> : null}
    <View style={styles.group}><SectionTitle>People</SectionTitle><Button size="compact" variant="outline" onPress={() => { setEditingPerson(null); setPersonName(''); setPersonContact(''); setPersonNote(''); setForm('person'); }}>Add person</Button>{people.length === 0 ? <Text style={ui.listMeta}>Add a person before creating a debt.</Text> : people.map((person) => { const hasDebt = debts.some((debt) => debt.personId === person.id); return <Card key={person.id}><View style={styles.personCard}><View style={styles.copy}><Text style={ui.listTitle}>{person.displayName}</Text><Text style={ui.listMeta}>{person.contact || person.note || 'No contact details'}</Text></View><View style={styles.personActions}><Button size="compact" variant="outline" onPress={() => { setEditingPerson(person); setPersonName(person.displayName); setPersonContact(person.contact ?? ''); setPersonNote(person.note ?? ''); setForm('person'); }}>Edit</Button><Button size="compact" variant="outline" disabled={hasDebt} onPress={() => void deletePerson(person)}>Delete</Button></View></View></Card>; })}</View>
    <DebtModal open={form === 'person'} title={editingPerson ? 'Edit person' : 'New person'} onClose={() => { setEditingPerson(null); setForm(null); }}><Field label="Display name" value={personName} onChangeText={setPersonName} /><Field label="Contact (optional)" value={personContact} onChangeText={setPersonContact} /><Field label="Note (optional)" value={personNote} onChangeText={setPersonNote} /><Button onPress={() => void savePerson()}>Save person</Button></DebtModal>
...
function useDebtStyles() { const { theme } = useAppTheme(); return useMemo(() => StyleSheet.create({ header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, marginBottom: 16 }, subtitle: { color: theme.mutedForeground, fontFamily: fontFamily.body, fontSize: 14, lineHeight: 20 }, group: { gap: 10, marginTop: 20 }, row: { flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }, copy: { flex: 1, minWidth: 0, gap: 2 }, amount: { fontFamily: fontFamily.display, fontSize: 16, fontWeight: '600', fontVariant: ['tabular-nums'] }, income: { color: theme.income }, expense: { color: theme.expense }, meta: { flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: 8, marginTop: 12 }, actions: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 14 }, personCard: { flexDirection: 'row', alignItems: 'center', gap: 10 }, personActions: { flexDirection: 'row', flexWrap: 'wrap', gap: 6 }, modalHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 22 }, form: { gap: 16, paddingBottom: 36 }, fieldLabel: { color: theme.foreground, fontFamily: fontFamily.body, fontSize: 14, fontWeight: '600', marginBottom: -8 }, chips: { flexDirection: 'row', flexWrap: 'wrap', marginTop: -8 } }), [theme]); }
