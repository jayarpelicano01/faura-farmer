...
describe('report period helpers', () => {
  ...
  it('creates a calendar-month range and previous calendar month', () => {
    ...
    expect(format(range.from, 'yyyy-MM-dd')).toBe('2026-08-01');
    expect(format(range.to, 'yyyy-MM-dd')).toBe('2026-08-31');
    expect(format(range.previousFrom, 'yyyy-MM-dd')).toBe('2026-07-01');
    expect(format(range.previousTo, 'yyyy-MM-dd')).toBe('2026-07-31');
  });
