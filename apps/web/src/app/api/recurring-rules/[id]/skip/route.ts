...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const securityFailure = await guardMutation(request, 'recurring-rule-skip', session.user.id);
  ...
  const result = await skipRecurringOccurrence(session.user.id, id, parsed.data.expectedDueDate);
