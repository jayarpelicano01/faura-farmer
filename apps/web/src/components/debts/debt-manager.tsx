...
export function DebtManager({ accounts }: { accounts: AccountOption[] }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {showPersonForm ? <PersonForm open={showPersonForm} person={editingPerson} onCancel={() => { setShowPersonForm(false); setResumeDebtAfterPerson(false); }} onSaved={async (person) => { setShowPersonForm(false); await load(); if (resumeDebtAfterPerson) { setNewDebtPersonId(person.id); setShowDebtForm(true); setResumeDebtAfterPerson(false); } }} /> : null}
      {showDebtForm ? <DebtForm open={showDebtForm} people={people} accounts={accounts} initialPersonId={newDebtPersonId} onCreatePerson={() => { setShowDebtForm(false); setEditingPerson(null); setResumeDebtAfterPerson(true); setShowPersonForm(true); }} onCancel={() => setShowDebtForm(false)} onSaved={async () => { setShowDebtForm(false); await load(); }} /> : null}
      {entry ? <EntryForm open={entry !== null} debt={entry.debt} kind={entry.kind} accounts={accounts} onCancel={() => setEntry(null)} onSaved={async () => { setEntry(null); await load(); }} /> : null}
    </div>
  );
}
...
function PersonForm({ open, person, onCancel, onSaved }: { open: boolean; person: PersonView | null; onCancel: () => void; onSaved: (person: PersonView) => Promise<void> }) {
  ...
  return <Dialog open={open} onOpenChange={(next) => { if (!next && !saving) onCancel(); }}><DialogContent className="sm:max-w-lg"><DialogHeader><DialogTitle className="font-display">{person ? 'Edit person' : 'New person'}</DialogTitle><DialogDescription>Add the person connected to the debt so their balance stays easy to identify.</DialogDescription></DialogHeader><form className="grid gap-4 sm:grid-cols-2" onSubmit={submit}><div className="space-y-1.5"><Label htmlFor="person-display-name">Display name</Label><Input id="person-display-name" required value={draft.displayName} placeholder="e.g. Maria Santos" onChange={(event) => setDraft({ ...draft, displayName: event.target.value })} /></div><div className="space-y-1.5"><Label htmlFor="person-contact">Contact <span className="font-normal text-muted-foreground">(optional)</span></Label><Input id="person-contact" value={draft.contact} placeholder="Phone or email" onChange={(event) => setDraft({ ...draft, contact: event.target.value })} /></div><div className="space-y-1.5 sm:col-span-2"><Label htmlFor="person-note">Note <span className="font-normal text-muted-foreground">(optional)</span></Label><Input id="person-note" value={draft.note} placeholder="Add a helpful detail" onChange={(event) => setDraft({ ...draft, note: event.target.value })} /></div><DialogFooter className="sm:col-span-2"><Button type="button" variant="outline" disabled={saving} onClick={onCancel}>Cancel</Button><Button type="submit" disabled={saving}>{saving ? 'Saving...' : 'Save person'}</Button></DialogFooter></form></DialogContent></Dialog>;
}

function DebtForm({ open, people, accounts, initialPersonId, onCreatePerson, onCancel, onSaved }: { open: boolean; people: PersonView[]; accounts: AccountOption[]; initialPersonId: string; onCreatePerson: () => void; onCancel: () => void; onSaved: () => Promise<void> }) {
  ...
  return <Dialog open={open} onOpenChange={(next) => { if (!next && !saving) onCancel(); }}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle className="font-display">New debt</DialogTitle><DialogDescription>Record money you lent or borrowed, with an optional cash account movement.</DialogDescription></DialogHeader><form className="grid gap-4 sm:grid-cols-2" onSubmit={submit}><div className="space-y-1.5 sm:col-span-2"><Label htmlFor="debt-person-search">Person</Label><Input id="debt-person-search" value={query} placeholder="Search people by name" onChange={(event) => setQuery(event.target.value)} /><select id="debt-person" required className="mt-2 h-10 w-full rounded-md border border-input bg-background px-3 text-sm text-foreground" value={personId} onChange={(event) => setPersonId(event.target.value)}><option value="">Select a person</option>{filteredPeople.map((person) => <option key={person.id} value={person.id}>{person.displayName}</option>)}</select><Button type="button" size="sm" variant="ghost" onClick={onCreatePerson}>Create new person</Button></div><div className="space-y-1.5"><Label htmlFor="debt-direction">Direction</Label><select id="debt-direction" className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm text-foreground" value={direction} onChange={(event) => setDirection(event.target.value as typeof direction)}><option value="receivable">They owe you</option><option value="payable">You owe them</option></select></div><div className="space-y-1.5"><Label htmlFor="debt-currency">Currency</Label><select id="debt-currency" className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm text-foreground" value={currency} onChange={(event) => { setCurrency(event.target.value as typeof currency); setOpeningAccountId(''); }}><option value="PHP">PHP</option><option value="USD">USD</option></select></div><div className="space-y-1.5"><Label htmlFor="debt-amount">Original principal ({currency})</Label><Input id="debt-amount" required type="number" min="0.01" step="0.01" value={amount} placeholder="e.g. 2500" onChange={(event) => setAmount(event.target.value)} /></div><div className="space-y-1.5"><Label htmlFor="debt-opened-at">Opened date</Label><Input id="debt-opened-at" required type="date" value={openedAt} onChange={(event) => setOpenedAt(event.target.value)} /></div><div className="space-y-1.5"><Label htmlFor="debt-due-date">Due date <span className="font-normal text-muted-foreground">(optional)</span></Label><Input id="debt-due-date" type="date" value={dueDate} onChange={(event) => setDueDate(event.target.value)} /></div><div className="space-y-1.5"><Label htmlFor="debt-note">Note <span className="font-normal text-muted-foreground">(optional)</span></Label><Input id="debt-note" value={note} placeholder="Add a helpful detail" onChange={(event) => setNote(event.target.value)} /></div><div className="space-y-1.5 sm:col-span-2"><Label htmlFor="debt-opening-account">Opening cash account <span className="font-normal text-muted-foreground">(optional)</span></Label><select id="debt-opening-account" className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm text-foreground" value={openingAccountId} onChange={(event) => setOpeningAccountId(event.target.value)}><option value="">No tracked cash movement</option>{accounts.filter((account) => account.currency === currency).map((account) => <option key={account.id} value={account.id}>{account.label} ({account.currency})</option>)}</select><p className="text-xs text-muted-foreground">Only same-currency accounts are available. A linked event changes the account balance but not spending reports.</p></div><DialogFooter className="sm:col-span-2"><Button type="button" variant="outline" disabled={saving} onClick={onCancel}>Cancel</Button><Button type="submit" disabled={saving || !personId}>{saving ? 'Saving...' : 'Create debt'}</Button></DialogFooter></form></DialogContent></Dialog>;
}

function EntryForm({ open, debt, kind, accounts, onCancel, onSaved }: { open: boolean; debt: DebtView; kind: 'payment' | 'adjustment'; accounts: AccountOption[]; onCancel: () => void; onSaved: () => Promise<void> }) {
  ...
  return <Dialog open={open} onOpenChange={(next) => { if (!next && !saving) onCancel(); }}><DialogContent className="sm:max-w-lg"><DialogHeader><DialogTitle className="font-display">{kind === 'payment' ? 'Record payment' : 'Record adjustment'}</DialogTitle><DialogDescription>{kind === 'payment' ? `Record a payment from ${debt.person.displayName}. Outstanding balance: ${formatMoney(debt.outstandingBalance, debt.currency)}.` : `Adjust the balance for ${debt.person.displayName}. Use a negative amount to reduce the principal.`}</DialogDescription></DialogHeader><form className="grid gap-4 sm:grid-cols-2" onSubmit={submit}><div className="space-y-1.5"><Label htmlFor="debt-entry-amount">Amount ({debt.currency})</Label><Input id="debt-entry-amount" required type="number" step="0.01" min={kind === 'payment' ? '0.01' : undefined} value={amount} placeholder={kind === 'payment' ? `Up to ${debt.outstandingBalance}` : 'Enter an amount'} onChange={(event) => setAmount(event.target.value)} /></div><div className="space-y-1.5"><Label htmlFor="debt-entry-date">Date</Label><Input id="debt-entry-date" required type="date" value={date} onChange={(event) => setDate(event.target.value)} /></div>{kind === 'payment' ? <><div className="space-y-1.5"><Label htmlFor="debt-payment-note">Note <span className="font-normal text-muted-foreground">(optional)</span></Label><Input id="debt-payment-note" value={note} placeholder="Add a helpful detail" onChange={(event) => setNote(event.target.value)} /></div><div className="space-y-1.5"><Label htmlFor="debt-payment-account">Cash account <span className="font-normal text-muted-foreground">(optional)</span></Label><select id="debt-payment-account" className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm text-foreground" value={accountId} onChange={(event) => setAccountId(event.target.value)}><option value="">Settled outside tracked accounts</option>{accounts.filter((account) => account.currency === debt.currency).map((account) => <option key={account.id} value={account.id}>{account.label} ({account.currency})</option>)}</select></div></> : <><div className="space-y-1.5"><Label htmlFor="debt-adjustment-reason">Reason</Label><select id="debt-adjustment-reason" className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm text-foreground" value={reason} onChange={(event) => setReason(event.target.value)}><option value="correction">Correction</option><option value="agreed_reduction">Agreed reduction</option><option value="partial_forgiveness">Partial forgiveness</option><option value="other">Other</option></select></div><div className="space-y-1.5"><Label htmlFor="debt-adjustment-other-reason">{reason === 'other' ? 'Describe the reason' : 'Additional details'}{reason !== 'other' && <span className="font-normal text-muted-foreground"> (optional)</span>}</Label><Input id="debt-adjustment-other-reason" required={reason === 'other'} value={otherReason} placeholder={reason === 'other' ? 'Describe the reason' : 'Add a helpful detail'} onChange={(event) => setOtherReason(event.target.value)} /></div></>}<DialogFooter className="sm:col-span-2"><Button type="button" variant="outline" disabled={saving} onClick={onCancel}>Cancel</Button><Button type="submit" disabled={saving}>{saving ? 'Saving...' : kind === 'payment' ? 'Save payment' : 'Save adjustment'}</Button></DialogFooter></form></DialogContent></Dialog>;
}
